# 👋 Hi, I'm Nehal Anurag

## 🧠 About Me

Mathematics professional with experience spanning **mathematical content development, AI/LLM evaluation, multimodal AI, data annotation, educational technology, and quality assurance**.

I have a strong background in Mathematics, with experience working across educational content, assessment authoring, textbook solutions, AI-generated response evaluation, multilingual search evaluation, and human-in-the-loop AI projects.

I also work with technical tools and technologies including **Python, LaTeX, MathType, Wolfram Mathematica, MATLAB, Generative AI, LLM evaluation, prompt engineering, and data annotation**.

---

## 🔬 Areas of Expertise

### 📐 Mathematics & Education

- Advanced Mathematics and mathematical problem solving
- K–12, undergraduate, and postgraduate mathematics
- Curriculum development and educational content creation
- Textbook solutions and step-by-step mathematical explanations
- Assessment and question authoring
- Mathematical content quality assurance
- Accessibility for mathematical content

### 🤖 Artificial Intelligence

- LLM / Generative AI evaluation
- AI-generated response evaluation
- Prompt and response evaluation
- RLHF and human-in-the-loop AI
- Multimodal AI evaluation
- AI search quality evaluation
- Search relevance and factuality assessment
- AI-generated image and transcription evaluation
- Multilingual AI evaluation
- Hindi language and localization evaluation
- Rubric design and quality assurance

### 💻 Technical Skills

- Python
- LaTeX
- MathType
- Wolfram Mathematica
- MATLAB
- Data Science fundamentals
- MySQL (Basic)
- Tableau (Basic)
- Microsoft Office

### 📊 Data & Quality

- Image and video annotation
- Bounding boxes, coordinates, timestamps, and object labeling
- Audio annotation
- Multimodal annotation
- Data quality assurance
- Information verification and fact-checking
- Factuality evaluation
- Linguistic quality evaluation
- Localization and cultural evaluation

---

## 💼 Professional Experience

My professional experience includes work as:

- **AI Trainer / Lingua Generalist** – Multimodal AI & LLM Evaluation
- **Data Annotator** – Image, video, audio, transcription, and AI response evaluation
- **AI Search / Hindi Culture Expert** – Multilingual Search Evaluation
- **Mathematics Subject Matter Expert** – Curriculum, textbook solutions, assessment, and content QA
- **Math Assessment Author** – Interactive mathematics assessments
- **Mathematics Teacher** – CBSE curriculum

I have worked on projects involving **Generative AI, LLM evaluation, multilingual search, multimodal annotation, educational technology, and mathematical content development**.

---

## 🛠️ Technologies & Tools

```text
Python
LaTeX
MathType
Wolfram Mathematica
MATLAB
Generative AI
LLM Evaluation
Prompt Engineering
Multimodal AI
Data Annotation
AI Search Evaluation
MySQL
Tableau
```

---

## 📂 Sample Work

Please find below a few samples of my mathematics and educational content creation work:

- [Sample 1](https://docs.google.com/document/d/1qej5sAtk3LO7t7QbSgXqmD1oCMxYmHXMWOWskkcTTmk/edit?usp=sharing)
- [Sample 2](https://docs.google.com/document/d/1bYFQ8pH4OvdPxqUXPFidYDsethn6H6pUh57x5PzPt_c/edit?usp=sharing)
- [Sample 3](https://docs.google.com/document/d/1A3xNH7oQ_yDdoRcsSMw2q9BCEORDg07i/edit?usp=sharing&ouid=107879152460616229508&rtpof=true&sd=true)
- [Sample 4](https://drive.google.com/file/d/14yNDqRpetcZfCXEgTd1tNYlOvz-tN4qJ/view?usp=sharing)

---

## 🚀 Projects

### NetBox – 16 Independent Bug Fixes

A software-engineering task involving the diagnosis and implementation of **16 independent bugs in the NetBox codebase**, with a structured environment, automated verification, and reference solution.

Repository: [netbox-16-bug-fix](https://github.com/Nehal-9/netbox-16-bug-fix)

---

## 🎓 Education

- **M.Sc. Mathematics** — University of Delhi
- **B.Sc. (Hons.) Mathematics** — University of Delhi
- **B.Ed.** — Maharshi Dayanand University

---

## 🌐 Languages

- **English** — C1 proficiency
- **Hindi** — Professional working proficiency

---

## 📫 Contact

- **Email:** nehalanurag18@gmail.com
- **LinkedIn:** [LinkedIn Profile](https://www.linkedin.com/in/nehal-anurag-a9b91570/)

---

⭐ **Mathematics • Artificial Intelligence • LLMs • Multimodal AI • Python • Data Quality • Search Evaluation • Educational Technology • Mathematical Computing**
